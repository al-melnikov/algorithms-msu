...
//шаблон
    ...
    std::cout << "allocated memory for matrix to sort " << std::endl;
    ...
    for (int arr_nomber=0; arr_nomber < 2*ARRAY_SIZE-1; arr_nomber++){
        ...
        FillMatrix(matrix_to_sort, &(*matrix)[arr_nomber]);
        std::cout << "filled matrix with " << arr_nomber << " array" << std::endl;

        auto start = std::chrono::high_resolution_clock::now();
        for (int time=0; time<SORT_TIMES; time++){
            ShellSort(&(*matrix_to_sort)[time]);
        }
        auto stop = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
        shellsort_file << arr_nomber << '\t' << (double)duration.count()/SORT_TIMES << std::endl;
        std::cout << arr_nomber << " array is sorted in " << (double)duration.count()/SORT_TIMES 
                            << " microseconds" << " with Shell sort" << std::endl;
    }
    ...
    for (int arr_nomber=0; arr_nomber < 2*ARRAY_SIZE-1; arr_nomber++){
        ...
        auto start = std::chrono::high_resolution_clock::now();
        for (int time=0; time<SORT_TIMES; time++){
            HeapSort(&(*matrix_to_sort)[time]);
        }
        auto stop = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
        heapsort_file << arr_nomber << '\t' << (double)duration.count()/SORT_TIMES << std::endl;
        std::cout << arr_nomber << " array is sorted in " << (double)duration.count()/SORT_TIMES 
                            << " microseconds" << " with Heap sort" << std::endl;
    }
